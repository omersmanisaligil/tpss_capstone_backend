...
public class MCDMCriteriaPoints {
    ...
    private static int determineC24(ProviderDTO provider){
        ...
        int grade;
        int accidentPerVehicle=0;
        try{
            accidentPerVehicle = totalAccidents/vehicles.size();
        }catch(ArithmeticException e){
            accidentPerVehicle = 0;
        }
        double score=0;

        if(accidentPerVehicle>1) score = 0.2;
        else if(accidentPerVehicle>0.8) score=0.4;
        else if(accidentPerVehicle>0.6) score=0.6;
        else if(accidentPerVehicle>0.4) score=0.8;
        else score=1;
